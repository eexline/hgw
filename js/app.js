...
async function generateSignal() {
    ...
    try {
        ...
        startSignalProgress(durationMs, startTime, async () => {
            ...
            syncDirectionStyles();
            try {
                const r = await fetch(`${API_URL}/signal?pair=${pair}&tf=${tf}`);
                if (r.ok) {
                    ...
                    if (newPrice === basePrice) {
                        ...
                        resultStatus = newPrice > basePrice ? 'WIN' : 'LOSE';
                    } else { // SELL
                        resultStatus = newPrice < basePrice ? 'WIN' : 'LOSE';
                    }
                    ...
                    if (data.id) {
                        fetch(`${API_URL}/signal/check?signal_id=${data.id}&current_price=${newPrice}`, {
                            method: 'POST'
                        }).catch(err => console.error("Failed to save result", err));
                    }

                    applySignalResultUI(resultStatus);
                }
            } catch (_) {}
    ...
    try {
        ...
        startSignalProgress(state.duration, state.startTime, async () => {
            ...
            if (state.type === 'OTC') {
                ...
                try {
                    const r = await fetch(`${API_URL}/signal?pair=${state.pair}&tf=${state.tf}`);
                    if (r.ok) {
                        const latest = await r.json();
                        const newPrice = Number(latest.price);
                        const basePrice = Number(state.price);
                        
                        let resultStatus = 'LOSE';
                        if (newPrice === basePrice) {
                            resultStatus = 'NEUTRAL';
                        } else if (state.signal === 'BUY') {
                            resultStatus = newPrice > basePrice ? 'WIN' : 'LOSE';
                        } else { 
                            resultStatus = newPrice < basePrice ? 'WIN' : 'LOSE';
                        }
                        ...
                        if (state.id) {
                            fetch(`${API_URL}/signal/check?signal_id=${state.id}&current_price=${newPrice}`, {
                                method: 'POST'
                            }).catch(err => console.error("Failed to save result", err));
                        }
